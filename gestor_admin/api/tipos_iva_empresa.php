...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $query_product = "SELECT
                        tipo_iva_empresa.id,
                        tipo_iva_empresa.nombre,
                        tipo_iva_empresa.letra_factura
...
if (isset($_GET['sort_order'])) {
        ...
        if (isset($_GET['param'])) {
            ...
            array_push($query_param, "tipo_iva_empresa.id=$id");
        }
        ...
        $query_total = "SELECT COUNT(*) AS total FROM tipo_iva_empresa";
        ...
        $query_product = $query_product . " ORDER BY tipo_iva_empresa. ".$order_by."  ".$sort_order." OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            array_push($response, $tipo_iva_empresa);
        }
    }
    //POST PRODUCTOS
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query_insert = "INSERT INTO tipo_iva_empresa (" . implode(", ", $param_insert) . ") VALUES (" . implode(", ", $param_values) . ")";
        $result = $con->query($query_insert);

        $query_id = "SELECT MAX(id) AS id FROM tipo_iva_empresa";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        ...
        $id = sanitizeInput($_GET['param']);
        $query_update = "UPDATE tipo_iva_empresa SET " . implode(", ", $param_update) . " WHERE tipo_iva_empresa.id = " . $id;
...
$id = sanitizeInput($_GET['param']);
        ...
        $query_delete = "DELETE FROM tipo_iva_empresa WHERE tipo_iva_empresa.id = :id";
