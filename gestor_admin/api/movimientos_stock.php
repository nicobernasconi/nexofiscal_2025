...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    $query_product = "SELECT
                        ...
                        movimientos_stock.observaciones
...
if (isset($_GET['sort_order'])) {
    ...
    if (isset($_GET['param'])) {
        ...
        array_push($query_param, "movimientos_stock.id=$id");
    }
    ...
    if (count($query_param) > 0) {
        $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param) . ") AND movimientos_stock.empresa_id = $empresa_id";
    }else{
        $query_product = $query_product . " WHERE movimientos_stock.empresa_id = $empresa_id";
    }

    //obtener el total de registros
    $query_total = "SELECT COUNT(*) AS total FROM movimientos_stock WHERE movimientos_stock.empresa_id = $empresa_id";
    ...
    $query_product = $query_product . " ORDER BY movimientos_stock.id LIMIT $limit OFFSET $offset";
    ...
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        ...
        array_push($response, $movimientos_stock);
    }
}
    //POST PRODUCTOS
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query_insert = "INSERT INTO movimientos_stock (" . implode(", ", $param_insert) . ") VALUES (" . implode(", ", $param_values) . ")";
        $result = $con->query($query_insert);


        $query_id = "SELECT MAX(id) AS id FROM movimientos_stock";
        $result_id = $con->query($query_id);
        $row_id = $result_id->fetch(PDO::FETCH_ASSOC);
        $id = $row_id['id'] ?? null;
        if ($result) {
            $response = array("status" => 201, "status_message" => "Moviemiento de Stock agregada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al agregar movimientos stock.", "id" => $id);
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        ...
        $id = sanitizeInput($_GET['param']);
        $query_update = "UPDATE movimientos_stock SET " . implode(", ", $param_update) . " WHERE movimientos_stock.id = " . $id;
        $result = $con->query($query_update);
        if ($result) {
            $response = array("status" => 201, "status_message" => "Moviemiento de Stock actualizada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al actualizar movimientos stock.", "id" => $id);
        }
    }
    //DELETE PRODUCTOS
...
$id = sanitizeInput($_GET['param']);
        ...
        $query_delete = "DELETE FROM movimientos_stock WHERE movimientos_stock.id = :id";
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                $response = array("status" => 201, "status_message" => "movimientos_stock eliminada correctamente.", "id" => $id);
            } else {
                $response = array("status" => 404, "status_message" => "No se encontró la movimientos_stock para eliminar.", "id" => $id);
            }
        } else {
            $response = array("status" => 400, "status_message" => "Error al eliminar movimientos_stock.", "id" => $id);
        }
    }
