...
if (isset($_GET['sort_order'])) {
    ...
    if (isset($_GET['param'])) {
        ...
        array_push($query_param, "subcategoria.id=$id");
    }
    if (isset($_GET['nombre'])) {
        $descripcion = sanitizeInput($_GET['nombre']);
        array_push($query_param, "nombre like '%$descripcion%'");
    }
    if (isset($_GET['se_imprime'])) {
        $se_imprime = sanitizeInput($_GET['se_imprime']);
        array_push($query_param, "se_imprime='$se_imprime'");
    }
    ...
    if (count($query_param) > 0) {
        $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param).") AND subcategoria.empresa_id = $empresa_id";
    }else{
        $query_product = $query_product . " WHERE subcategoria.empresa_id = $empresa_id";
    }

    //obtener el total de registros
    $query_total = "SELECT COUNT(*) AS total FROM subcategoria WHERE subcategoria.empresa_id = $empresa_id";
    ...
    $query_product = $query_product . " ORDER BY subcategoria. ".$order_by."  ".$sort_order." OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
    ...
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        ...
        array_push($response, $subcategoria);
    }
}
    //POST PRODUCTOS
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query_insert = "INSERT INTO subcategoria (" . implode(", ", $param_insert) . ") VALUES (" . implode(", ", $param_values) . ")";
        $result = $con->query($query_insert);

        $query_id = "SELECT MAX(id) AS id FROM subcategoria";
        $result_id = $con->query($query_id);
        $row_id = $result_id->fetch(PDO::FETCH_ASSOC);
        $id = $row_id['id'] ?? null;
        if ($result) {
            $response = array("status" => 201, "status_message" => "Subcategoria agregada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al agregar subcategoria.", "id" => $id);
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        ...
        $id = sanitizeInput($_GET['param']);
        $query_update = "UPDATE subcategoria SET " . implode(", ", $param_update) . " WHERE subcategoria.id = " . $id;
        $result = $con->query($query_update);
        if ($result) {
            $response = array("status" => 201, "status_message" => "Subcategoria actualizada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al actualizar subcategoria.", "id" => $id);
        }
    }
    //DELETE PRODUCTOS
...
$id = sanitizeInput($_GET['param']);
        ...
        $query_delete = "DELETE FROM subcategoria WHERE subcategoria.id = :id";
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                $response = array("status" => 201, "status_message" => "Subcategoria eliminada correctamente.", "id" => $id);
            } else {
                $response = array("status" => 404, "status_message" => "No se encontró la subcategoria para eliminar.", "id" => $id);
            }
        } else {
            $response = array("status" => 400, "status_message" => "Error al eliminar subcategoria.", "id" => $id);
        }
    }
