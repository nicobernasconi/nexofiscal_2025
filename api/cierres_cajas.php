...
if (isset($_GET['sort_order'])) {
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param).") AND cierres_cajas.empresa_id = $empresa_id";
        }else{
            $query_product = $query_product . " WHERE cierres_cajas.empresa_id = $empresa_id";
        }


        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM cierres_cajas WHERE cierres_cajas.empresa_id = $empresa_id";
