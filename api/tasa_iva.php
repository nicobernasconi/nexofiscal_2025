...
if (isset($_GET['sort_order'])) {
        ...
        if (isset($_GET['param'])) {
            ...
            array_push($query_param, "tasa_iva.id=$id");
        }
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param).") AND tasa_iva.empresa_id = $empresa_id";
        }else{
            $query_product = $query_product . " WHERE tasa_iva.empresa_id = $empresa_id";
        }

        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM tasa_iva WHERE tasa_iva.empresa_id = $empresa_id";
        ...
        $query_product = $query_product . " ORDER BY tasa_iva. ".$order_by."  ".$sort_order." OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query_insert = "INSERT INTO tasa_iva (" . implode(", ", $param_insert) . ") VALUES (" . implode(", ", $param_values) . ")";
        $result = $con->query($query_insert);

        $query_id = "SELECT MAX(id) AS id FROM tasa_iva";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        ...
        $id = sanitizeInput($_GET['param']);
        $query_update = "UPDATE tasa_iva SET " . implode(", ", $param_update) . " WHERE tasa_iva.id = " . $id;
...
$id = sanitizeInput($_GET['param']);
        ...
        $query_delete = "DELETE FROM tasa_iva WHERE tasa_iva.id = :id";
