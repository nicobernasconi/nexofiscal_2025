...
familias";
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param) . ") AND familias.empresa_id = $empresa_id";
        } else {
            $query_product = $query_product . " WHERE familias.empresa_id = $empresa_id";
        }

        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM familias WHERE familias.empresa_id = $empresa_id";
