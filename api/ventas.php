...
try {
    ...
    WHERE
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " and (" . implode(" OR ", $query_param) . ") AND comprobantes.empresa_id = $empresa_id";
        } else {
            $query_product = $query_product . " and comprobantes.empresa_id = $empresa_id";
        }

        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM comprobantes WHERE comprobantes.fecha_baja IS NULL AND comprobantes.empresa_id = $empresa_id";
        ...
        $query_product = $query_product . " ORDER BY comprobantes.id LIMIT $limit OFFSET $offset";
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            $clientes = array(
                "id" => $row['cliente_id'],
                "nro_cliente" => $row['cliente_numero'],
