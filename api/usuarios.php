...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param) . ") AND usuarios.empresa_id = $empresa_id";
        } else {
            $query_product = $query_product . " WHERE usuarios.empresa_id = $empresa_id";
        }

        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM usuarios WHERE usuarios.empresa_id = $empresa_id";
