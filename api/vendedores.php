...
if (isset($_GET['sort_order'])) {
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param).") AND vendedores.empresa_id = $empresa_id";
        }else{
            $query_product = $query_product . " WHERE vendedores.empresa_id = $empresa_id";
        }

        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM vendedores WHERE vendedores.empresa_id = $empresa_id";
        ...
        $query_product = $query_product . " ORDER BY vendedores. ".$order_by."  ".$sort_order." LIMIT $limit OFFSET $offset";
