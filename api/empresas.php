...
try {
    $filtro_distribuidor = "0=1";
    if($empresa_id=999999999){
       $filtro_distribuidor = " empresas.id IN (select empresa_id from distribuidores_empresas where distribuidor_id=$usuario_id)";
    }
...
if (isset($_GET['sort_order'])) {
    ...
    if (isset($_GET['logo'])) {
        ...
        array_push($query_param, "logo like '%$descripcion%'");
    }

    if (isset($_GET['direccion'])) {
        $descripcion = sanitizeInput($_GET['direccion']);
        array_push($query_param, "direccion like '%$descripcion%'");
    }
    if (isset($_GET['telefono'])) {
        $descripcion = sanitizeInput($_GET['telefono']);
        array_push($query_param, "telefono like '%$descripcion%'");
    }
    if (isset($_GET['tipo_iva'])) {
        $descripcion = sanitizeInput($_GET['tipo_iva']);
        array_push($query_param, "tipo_iva like '%$descripcion%'");
    }
    if (isset($_GET['cuit'])) {
        $descripcion = sanitizeInput($_GET['cuit']);
        array_push($query_param, "cuit like '%$descripcion%'");
    }
    if (isset($_GET['responsable'])) {
        $descripcion = sanitizeInput($_GET['responsable']);
        array_push($query_param, "responsable like '%$descripcion%'");
    }
    if (isset($_GET['email'])) {
        $descripcion = sanitizeInput($_GET['email']);
        array_push($query_param, "email like '%$descripcion%'");
    }
    if (isset($_GET['rubro_id'])) {
        $descripcion = sanitizeInput($_GET['rubro_id']);
        array_push($query_param, "rubro_id like '%$descripcion%'");
    }
    if (isset($_GET['fecha_inicio_actividades'])) {
        $descripcion = sanitizeInput($_GET['fecha_inicio_actividades']);
        array_push($query_param, "fecha_inicio_actividades like '%$descripcion%'");
    }
    ...
    if (count($query_param) > 0) {
        $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param) . ") AND ".$filtro_distribuidor;
    }
    ...
    $query_product = $query_product . " ORDER BY empresas. ".$order_by."  ".$sort_order." OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
