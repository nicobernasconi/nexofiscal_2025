...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $query_product = "SELECT
                            ...
                            COALESCE ( ( SELECT SUM( precio_costo * cantidad ) FROM compras WHERE proveedores.id = compras.proveedor_id and nro_factura <> '0' ), 0 ) - COALESCE ( ( SELECT SUM( monto ) FROM pagos WHERE proveedores.id = pagos.proveedor_id ), 0 ) AS saldo_actual,
                            localidad.nombre,
                            tipo_iva.nombre,
...
if (isset($_GET['sort_order'])) {
        ...
        $query_product = $query_product . " ORDER BY proveedores. ".$order_by."  ".$sort_order." OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
