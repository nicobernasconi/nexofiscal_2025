...
// URL de la API
$url = $ruta . 'distribuidores_admin/api/familias/';
...
try {
    ...
    foreach ($data as $item) {
        $boton_borrar='';
        $boton_modificar='';
        if (in_array('modificar', $permisos_asignados['familias'])) {
        $boton_borrar = '<button class="btn-cancelar btn-eliminar-familia" data-id="'.$item['id'].'"><i class="fas fa-trash"></i></button>';
        }
         if (in_array('modificar', $permisos_asignados['familias'])) {
        $boton_modificar = '<button class="btn-guardar btn-seleccionar-familia" data-id="'.$item['id'].'" ><i class="fas fa-pencil"></i></button>';
         }
        $formattedItem = [
            'id' => $item['id'],
            'nombre' => $item['nombre'],
