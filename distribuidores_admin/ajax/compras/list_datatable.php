...
// URL de la API
$url = $ruta . 'distribuidores_admin/api/compras/';
...
$params = [
    'headers' => [
        ...
        'nro_factura' => isset($searchValue) ? $searchValue :null,
...
try {
    ...
    foreach ($data as $item) {
        $boton_borrar='';
        $boton_modificar='';
        if (in_array('modificar', $permisos_asignados['formas_pago'])) {
        $boton_borrar = '<button class="btn-cancelar btn-eliminar-forma-pago" data-id="'.$item['id'].'"><i class="fas fa-trash"></i></button>';
        }
         if (in_array('modificar', $permisos_asignados['formas_pago'])) {
        $boton_modificar = '<button class="btn-guardar btn-seleccionar-forma-pago" data-id="'.$item['id'].'" ><i class="fas fa-pencil"></i></button>';
         }
        $formattedItem = [
            'id' => $item['id'],
            'fecha'=> $item['fecha'],
