...
// URL de la API
$url = $ruta . 'distribuidores_admin/api/gastos/';
...
try {
    ...
    foreach ($data as $item) {
        $boton_borrar='';
        $boton_modificar='';
        if (in_array('modificar', $permisos_asignados['tipo'])) {
        $boton_borrar = '<button class="btn-cancelar btn-eliminar-tipo" data-id="'.$item['id'].'"><i class="fas fa-trash"></i></button>';
        }
         if (in_array('modificar', $permisos_asignados['tipo'])) {
        $boton_modificar = '<button class="btn-guardar btn-seleccionar-tipo" data-id="'.$item['id'].'" ><i class="fas fa-pencil"></i></button>';
         }
        $formattedItem = [
            'id' => $item['id'],
            'nombre' => $item['nombre'],
