...
// URL de la API
$url = $ruta . 'distribuidores_admin/api/tipos_documento/';
...
try {
    ...
    foreach ($data as $item) {
        $boton_borrar='';
        $boton_modificar='';
        if (in_array('modificar', $permisos_asignados['tipo_documento'])) {
        $boton_borrar = '<button class="btn-cancelar btn-eliminar-tipo-documento" data-id="'.$item['id'].'"><i class="fas fa-trash"></i></button>';
        }
         if (in_array('modificar', $permisos_asignados['tipo_documento'])) {
        $boton_modificar = '<button class="btn-guardar btn-seleccionar-tipo-documento" data-id="'.$item['id'].'" ><i class="fas fa-pencil"></i></button>';
         }
        $formattedItem = [
            'id' => $item['id'],
            'nombre' => $item['nombre'],
