...
// URL de la API
$url = $ruta . 'distribuidores_admin/api/promociones/';
...
try {
    ...
    foreach ($data as $item) {
        ...
        $boton_borrar = '<button class="btn-cancelar btn-eliminar-promocion" data-id="'.$item['id'].'"><i class="fas fa-trash"></i></button>';
        }
         if (in_array('modificar', $permisos_asignados['tipo_iva'])) {
        $boton_modificar = '<button class="btn-guardar btn-seleccionar-promocion" data-id="'.$item['id'].'" ><i class="fas fa-pencil"></i></button>';
         }
        $formattedItem = [
            'id' => $item['id'],
            'nombre' => $item['nombre'],
