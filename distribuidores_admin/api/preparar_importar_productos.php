...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($encabezado != ["CODIGO", "DESCRIPCION", "PRECIO1", "PRECIO2", "PRECIO3", "TASA_IVA"]) {
            throw new Exception("El archivo no tiene el formato correcto (Formato del encabezado)", 1);
        }
        //controlar si la cantidad de columnas es correcta
        foreach ($data as $key => $value) {
            if (count($value) != 6) {
                throw new Exception("El archivo no tiene el formato correcto (Cantidad de columnas)", 1);
            }
        }
