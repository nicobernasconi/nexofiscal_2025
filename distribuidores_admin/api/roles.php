...
if (isset($_GET['sort_order'])) {
    ...
    if (isset($_GET['param'])) {
        ...
        array_push($query_param, "roles.id=$id");
    }
    ...
    if (count($query_param) > 0) {
        $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param).") AND roles.empresa_id = $empresa_id";
    }else{
        $query_product = $query_product . " WHERE roles.empresa_id = $empresa_id";
    }

    //obtener el total de registros
    $query_total = "SELECT COUNT(*) AS total FROM roles WHERE roles.empresa_id = $empresa_id";
    ...
    $query_product = $query_product . " ORDER BY roles. ".$order_by."  ".$sort_order." OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query_insert = "INSERT INTO roles (" . implode(", ", $param_insert) . ") VALUES (" . implode(", ", $param_values) . ")";
        $result = $con->query($query_insert);

        $query_id = "SELECT MAX(id) AS id FROM roles";
        $result_id = $con->query($query_id);
        $row_id = $result_id->fetch(PDO::FETCH_ASSOC);
        $id = $row_id['id'] ?? null;
        if ($result) {
            $response = array("status" => 201, "status_message" => "roles agregada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al agregar roles.", "id" => $id);
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        ...
        $id = sanitizeInput($_GET['param']);
        $query_update = "UPDATE roles SET " . implode(", ", $param_update) . " WHERE roles.id = " . $id;
        $result = $con->query($query_update);
        if ($result) {
            $response = array("status" => 201, "status_message" => "rol actualizada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al actualizar roles.", "id" => $id);
        }
    }
    //DELETE PRODUCTOS
...
$id = sanitizeInput($_GET['param']);
        ...
        $query_delete = "DELETE FROM roles WHERE roles.id = :id";
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                $response = array("status" => 201, "status_message" => "rol eliminada correctamente.", "id" => $id);
            } else {
                $response = array("status" => 404, "status_message" => "No se encontró la rol para eliminar.", "id" => $id);
            }
        } else {
            $response = array("status" => 400, "status_message" => "Error al eliminar roles.", "id" => $id);
        }
    }
