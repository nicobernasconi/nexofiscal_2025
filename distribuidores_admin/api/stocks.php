...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if (isset($_GET['sucursal_id'])) {
            $stock_actual = " (select stock_actual from productos_stock where productos_stock.producto_id = productos.id and productos_stock.sucursal_id = " . $_GET['sucursal_id'] . ") as stock";
        } else {
            $stock_actual = " 0 as stock";
        }
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param) . ") AND productos.empresa_id = $empresa_id AND productos.codigo <> ''";
        } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $result = $con->query($query_stock);
        if ($result) {
            $response = array("status" => 201, "status_message" => "Stock actualizado correctamente.");
        } else {
            $response = array("status" => 500, "status_message" => "Error al actualizar el stock.");
        }
    }
