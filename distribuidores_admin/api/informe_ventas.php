...
try {
    ...
    WHERE
        ...
        if (count($query_param) > 0) {
            ...
            $query_product_resumen = "SELECT SUM(total) AS total_ventas, SUM(importe_iva) AS total_iva, COUNT(*) AS total_comprobantes FROM comprobantes LEFT JOIN distribuidores_empresas ON comprobantes.empresa_id = distribuidores_empresas.empresa_id WHERE comprobantes.fecha_baja IS NULL";
        }
