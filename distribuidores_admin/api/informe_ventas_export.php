...
try {
    ...
    FROM
        ...
        LEFT JOIN sucursales ON comprobantes.sucursal_id = sucursales.id
        ...
        LEFT JOIN empresas ON distribuidores_empresas.empresa_id = empresas.id
    ...
    WHERE
        ...
        if (isset($_GET['empresa_id'])) {
            $distribuidor_id = sanitizeInput($_GET['empresa_id']);
        ...
        if (count($query_param) > 0) {
            ...
            $query_product_resumen = "SELECT SUM(total) AS total_ventas, SUM(importe_iva) AS total_iva, COUNT(*) AS total_comprobantes FROM comprobantes LEFT JOIN distribuidores_empresas ON comprobantes.empresa_id = distribuidores_empresas.empresa_id WHERE comprobantes.fecha_baja IS NULL and (" . implode(" AND ", $query_param) . ") ";
        } else {
            $query_product_resumen = "SELECT SUM(total) AS total_ventas, SUM(importe_iva) AS total_iva, COUNT(*) AS total_comprobantes FROM comprobantes LEFT JOIN distribuidores_empresas ON comprobantes.empresa_id = distribuidores_empresas.empresa_id WHERE comprobantes.fecha_baja IS NULL AND distribuidores_empresas.distribuidor_id=$distribuidor_id";
        }
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            $comprobante = array(
                ...
                "fecha" => $row['fecha'],
                "hora" => $row['hora'],
                "numero_factura" => $row['tipo_comprobante_nombre'] . ' ' . $letra . ' ' . sprintf("%04d %08d", $row['punto_venta'], $numero),
                "total" => $row['total'],
        ...
        if($type=='excel'){
            ...
            $sheet->setCellValue('A1', 'Numero de Factura');
            $sheet->setCellValue('B1', 'Cliente');
            $sheet->setCellValue('C1', 'Tipo de IVA');
            $sheet->setCellValue('D1', 'CUIT');
            $sheet->setCellValue('E1', 'Fecha');
            $sheet->setCellValue('F1', 'Hora');
            $sheet->setCellValue('G1', 'Total');
            $sheet->setCellValue('H1', 'Importe IVA');
            $sheet->setCellValue('I1', 'Vendedor');
            $sheet->setCellValue('J1', 'Sucursal');
            $sheet->setCellValue('K1', 'Empresa');
            $i = 2;
            foreach ($ventas as $venta) {
                $sheet->setCellValue('A' . $i, $venta['numero_factura']);
                $sheet->setCellValue('B' . $i, $venta['cliente_id']);
                $sheet->setCellValue('C' . $i, $venta['tipo_iva_id']);
                $sheet->setCellValue('D' . $i, $venta['cuit']);
                $sheet->setCellValue('E' . $i, $venta['fecha']);
                $sheet->setCellValue('F' . $i, $venta['hora']);
                $sheet->setCellValue('G' . $i, $venta['total']);
                $sheet->setCellValue('H' . $i, $venta['importe_iva']);
                $sheet->setCellValue('I' . $i, $venta['vendedor_id']);
                $sheet->setCellValue('J' . $i, $venta['sucursal_id']);
                $sheet->setCellValue('K' . $i, $venta['empresa_id']);
                $i++;
            }
            //completo la ultima fila con los totales
            $sheet->setCellValue('A' . $i, 'Total');
            $sheet->setCellValue('G' . $i, '$'.$resumen['total_ventas']);
            $sheet->setCellValue('H' . $i, '$'.$resumen['total_iva']);

            //pongo los bordes a las celdas
            $styleArray = [
                'borders' => [
                    'allBorders' => [
            ...
            //aplico estilo a la fila de los totales y a la cabezera
            ...
            $sheet->getStyle('A' . $i . ':K' . $i)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFA0A0A0');
            //auto ajusto el ancho de las columnas
         foreach (range('A', 'K') as $columnID) {
            ...
            $filename = 'ventas' . $distribuidor_id . '.xlsx';
            ...
            $filename = 'ventas' . $distribuidor_id  . '.pdf';
            $filepath = './../comprobantes/'.$filename;
            $pdf = new FPDF();
           //crear paginas en horizontal
            ...
            $pdf->Cell(0, 10, 'Listado de Ventas - Periodo: ' . date('d-m-Y', strtotime($fecha_inicio)) . ' al ' . date('d-m-Y', strtotime($fecha_fin)), 0, 1, 'C');
            //agregar tabla
            $pdf->SetFont('Arial', 'B', 9);
            $pdf->Cell(30, 6, 'Numero Factura', 1, 0, 'C');
            $pdf->Cell(30, 6, 'Cliente', 1, 0, 'C');
            $pdf->Cell(30, 6, 'Tipo de IVA', 1, 0, 'C');
            $pdf->Cell(20, 6, 'CUIT', 1, 0, 'C');
            $pdf->Cell(20, 6, 'Fecha', 1, 0, 'C');
            $pdf->Cell(20, 6, 'Hora', 1, 0, 'C');
            $pdf->Cell(20, 6, 'Total', 1, 0, 'C');
            $pdf->Cell(20, 6, 'Importe IVA', 1, 0, 'C');
            $pdf->Cell(30, 6, 'Vendedor', 1, 0, 'C');
            $pdf->Cell(40, 6, 'Sucursal', 1, 0, 'C');
            $pdf->Cell(20, 6, 'Empresa', 1, 1, 'C');
            $pdf->SetFont('Arial', '', 9);
            $pdf->SetFontSize(6);
            foreach ($ventas as $venta) {
                $pdf->Cell(30, 6, $venta['numero_factura'], 1, 0, 'C');
                $pdf->Cell(30, 6, $venta['cliente_id'], 1, 0, 'C');
                $pdf->Cell(30, 6, $venta['tipo_iva_id'], 1, 0, 'C');
                $pdf->Cell(20, 6, $venta['cuit'], 1, 0, 'C');
                $pdf->Cell(20, 6, $venta['fecha'], 1, 0, 'C');
                $pdf->Cell(20, 6, $venta['hora'], 1, 0, 'C');
                $pdf->Cell(20, 6, $venta['total'], 1, 0, 'C');
                $pdf->Cell(20, 6, $venta['importe_iva'], 1, 0, 'C');
                $pdf->Cell(30, 6, $venta['vendedor_id'], 1, 0, 'C');
                $pdf->Cell(40, 6, $venta['sucursal_id'], 1, 0, 'C');
                $pdf->Cell(20, 6, $venta['empresa_id'], 1, 1, 'C');
            }

            $pdf->SetFont('Arial', 'B', 9);
            $pdf->Cell(150, 6, 'Total', 1, 0, 'L');
            $pdf->Cell(20, 6, '$'.$resumen['total_ventas'], 1, 0, 'C');
            $pdf->Cell(20, 6, '$'.$resumen['total_iva'], 1, 0, 'C');
            $pdf->Cell(90, 6, '', 1, 1, 'C');
