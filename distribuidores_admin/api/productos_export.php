...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $query_product = $query_product . " ORDER BY  " . ' p.' . $order_by . "  " . $sort_order ;
        ...
        if($type=='pdf'){
           ...
           //crear paginas en horizontal
            ...
            $pdf->Cell(30, 8, 'Balanza', 1, 0, 'C');
            $pdf->Cell(30, 8, 'IVA', 1, 0, 'C');
            $pdf->Cell(20, 8, 'Stock', 1, 0, 'C');
            $pdf->Cell(30, 8, 'Stock Minimo', 1, 0, 'C');
            $pdf->Cell(30, 8, 'Stock Pedido', 1, 0, 'C');
            $pdf->Ln();
            foreach ($formattedData as $item) {
                ...
                $pdf->Cell(30, 8, $item['producto_balanza'], 1, 0, 'C');
                $pdf->Cell(30, 8, $item['tasa_iva'], 1, 0, 'C');
                $pdf->Cell(20, 8, $item['stock'], 1, 0, 'C');
                $pdf->Cell(30, 8, $item['stock_minimo'], 1, 0, 'C');
                $pdf->Cell(30, 8, $item['stock_pedido'], 1, 0, 'C');
                $pdf->Ln();
            }
