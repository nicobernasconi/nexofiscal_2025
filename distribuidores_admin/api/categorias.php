...
if (isset($_GET['sort_order'])) {
    ...
    if (isset($_GET['param'])) {
        ...
        array_push($query_param, "categoria.id=$id");
    }
    ...
    if (count($query_param) > 0) {
        $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param).") AND categoria.empresa_id = $empresa_id";
    }else{
        $query_product = $query_product . " WHERE categoria.empresa_id = $empresa_id";
    }

    //obtener el total de registros
    $query_total = "SELECT COUNT(*) AS total FROM categoria WHERE categoria.empresa_id = $empresa_id";
    ...
    $query_product = $query_product . " ORDER BY categoria. ".$order_by."  ".$sort_order." OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query_insert = "INSERT INTO categoria (" . implode(", ", $param_insert) . ") VALUES (" . implode(", ", $param_values) . ")";
        $result = $con->query($query_insert);

        $query_id = "SELECT MAX(id) AS id FROM categoria";
        $result_id = $con->query($query_id);
        $row_id = $result_id->fetch(PDO::FETCH_ASSOC);
        $id = $row_id['id'] ?? null;
        if ($result) {
            $response = array("status" => 201, "status_message" => "Categoria agregada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al agregar categoria.", "id" => $id);
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        ...
        $id = sanitizeInput($_GET['param']);
        $query_update = "UPDATE categoria SET " . implode(", ", $param_update) . " WHERE categoria.id = " . $id;
        $result = $con->query($query_update);
        if ($result) {
            $response = array("status" => 201, "status_message" => "Categoria actualizada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al actualizar categoria.", "id" => $id);
        }
    }
    //DELETE PRODUCTOS
...
$id = sanitizeInput($_GET['param']);
        ...
        $query_delete = "DELETE FROM categoria WHERE categoria.id = :id";
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                $response = array("status" => 201, "status_message" => "Categoria eliminada correctamente.", "id" => $id);
            } else {
                $response = array("status" => 404, "status_message" => "No se encontró la categoria para eliminar.", "id" => $id);
            }
        } else {
            $response = array("status" => 400, "status_message" => "Error al eliminar categoria.", "id" => $id);
        }
    }
