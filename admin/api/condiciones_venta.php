...
if (isset($_GET['sort_order'])) {
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param).") AND condicion_venta.empresa_id = $empresa_id";
        }else{
            $query_product = $query_product . " WHERE condicion_venta.empresa_id = $empresa_id";
        }


        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM condicion_venta WHERE condicion_venta.empresa_id = $empresa_id";
        ...
        $query_product = $query_product . " ORDER BY condicion_venta. ".$order_by."  ".$sort_order." OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            array_push($response, $condicion_venta);
        }
    }
    //POST PRODUCTOS
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query_insert = "INSERT INTO condicion_venta (" . implode(", ", $param_insert) . ") VALUES (" . implode(", ", $param_values) . ")";
        $result = $con->query($query_insert);

        $query_id = "SELECT MAX(id) AS id FROM condicion_venta";
        $result_id = $con->query($query_id);
        $row_id = $result_id->fetch(PDO::FETCH_ASSOC);
        $id = $row_id['id'] ?? null;
        if ($result) {
            $response = array("status" => 201, "status_message" => "Condicion Venta agregada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al agregar Condicion Venta .", "id" => $id);
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        ...
        $id = sanitizeInput($_GET['param']);
        $query_update = "UPDATE condicion_venta SET " . implode(", ", $param_update) . " WHERE condicion_venta.id = " . $id;
        $result = $con->query($query_update);
        if ($result) {
            $response = array("status" => 201, "status_message" => "Condicion Venta  actualizada correctamente.", "id" => $id);
        } else {
            $response = array("status" => 400, "status_message" => "Error al actualizar Condicion Venta .", "id" => $id);
        }
    }
    //DELETE PRODUCTOS
...
$id = sanitizeInput($_GET['param']);
        ...
        $query_delete = "DELETE FROM condicion_venta WHERE condicion_venta.id = :id";
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                $response = array("status" => 201, "status_message" => "Condicion Venta  eliminada correctamente.", "id" => $id);
            } else {
                $response = array("status" => 404, "status_message" => "No se encontró la Condicion Venta  para eliminar.", "id" => $id);
            }
        } else {
            $response = array("status" => 400, "status_message" => "Error al eliminar Condicion Venta .", "id" => $id);
        }
    }
