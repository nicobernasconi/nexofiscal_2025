...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        $query_product = "SELECT
                        ...
                        comprobantes.importe_iva_1 AS importe_iva_1,
                        comprobantes.importe_iva_2 AS importe_iva_2,
                        comprobantes.importe_iva_3 AS importe_iva_3,
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE  (" . implode(" OR ", $query_param) . ') and comprobantes.empresa_id=' . $empresa_id;
        } else {
            $query_product = $query_product . " WHERE comprobantes.empresa_id=" . $empresa_id;
        }
        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM comprobantes WHERE comprobantes.empresa_id=" . $empresa_id;
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            $comprobantes = array(
                ...
                'importe_iva_1' => $row['importe_iva_1'],
                'importe_iva_2' => $row['importe_iva_2'],
                'importe_iva_3' => $row['importe_iva_3'],
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        //obtener el id del tipo compronbante
        $tipo_comprobante = $data['tipo_comprobante_id'];
        //si es 1 es comprobantes si es 3 es pedido
        $tipo_comprobante_nombre = ($tipo_comprobante == 1) ? 'FACTURA' : 'PEDIDO';
        //OBTENER EL ID DEL TIPO DE COMPROBANTE DE LA EMPRESA on id $empresa_id
        $query_tipo_comprobante = "SELECT id FROM tipo_comprobante WHERE nombre='$tipo_comprobante_nombre' AND empresa_id=$empresa_id";
        $result_tipo_comprobante = $con->query($query_tipo_comprobante);
        $row_tipo_comprobante = $result_tipo_comprobante->fetch(PDO::FETCH_ASSOC);
        $tipo_comprobante_id = $row_tipo_comprobante['id'] ?? null;
        $data['tipo_comprobante_id'] = $tipo_comprobante_id;

        //cambiar el parametro para que le asigne el numero de factura sumandole 1 a la ultima factura de esa empresa
        $query_numero_factura = "SELECT MAX(numero_factura) AS numero_factura FROM comprobantes WHERE empresa_id=$empresa_id";
        $result_numero_factura = $con->query($query_numero_factura);
        $row_numero_factura = $result_numero_factura->fetch(PDO::FETCH_ASSOC);


        $data['numero_factura'] = $row_numero_factura['numero_factura'] + 1;
        if ($tipo_comprobante != 1) {
            $data['numero_factura'] = 0;
        }
        ...
        if ($result) {
            $response = array("status" => 201, "status_message" => "comprobantes agregada correctamente.", "id" => $id, "numero_factura" => $numero_factura);
        } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'DELETE') {
        ...
        $query_delete = "DELETE FROM comprobantes WHERE comprobantes.id = :id";
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                $response = array("status" => 201, "status_message" => "comprobantes  eliminada correctamente.", "id" => $id);
            } else {
                $response = array("status" => 404, "status_message" => "No se encontró la comprobantes  para eliminar.", "id" => $id);
            }
        } else {
            $response = array("status" => 400, "status_message" => "Error al eliminar comprobantes .", "id" => $id);
        }
    }
