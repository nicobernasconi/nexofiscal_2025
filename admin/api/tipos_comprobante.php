...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if (count($query_param) > 0) {
            $query_product = $query_product . " WHERE (" . implode(" OR ", $query_param) . ") AND tipo_comprobante.empresa_id = $empresa_id";
        } else {
            $query_product = $query_product . " WHERE tipo_comprobante.empresa_id = $empresa_id";
        }

        //obtener el total de registros
        $query_total = "SELECT COUNT(*) AS total FROM tipo_comprobante WHERE tipo_comprobante.empresa_id = $empresa_id";
        ...
        $query_product = $query_product . " ORDER BY tipo_comprobante. " . $order_by . "  " . $sort_order . " OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
